...
<script type="text/javascript">
	...
	function commentsDelete() {
		...
		if (confirm("确认要删除这些评论吗？")) {
			$.post("comment?action=delete",{commentId:commentIds},
					function(result){
						var result = eval('('+result+')');
						if(result.success){
							alert("成功删除"+result.delNums+"条数据");
							window.location.href="comment?action=backList";
						}else{
	...
	function commentDelete(commentIds) {
		...
		<table class="table table-hover table-bordered">
			...
			<c:forEach var="commentBack" items="${commentBackList }" varStatus="status">
				<tr>
					...
					<td><a title="${commentBack.newsName }" target="_blank" href="news?action=show&newsId=${commentBack.newId }">${fn:substring(commentBack.newsName,0,15) }...</a></td>
					<td><fmt:formatDate value="${commentBack.commentDate }" type="date" pattern="yyyy-MM-dd HH:mm:ss"/></td>
